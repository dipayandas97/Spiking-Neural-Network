...
class SNN_Model:
    ...
    def predict(self, input_signal):                              #signal is a 2D matrix shaped: (feature,timesteps)
        ...
        activations_list[0][:,:] = input_signal[:,:]              #First activation map is the input activation map            
     
        #initialize all neurons in the model
        for l in range(self.num_layers):
        ...
        for t in range(simulation_time):
            for l in range(self.num_layers):
                ...
                for n_id,neuron in enumerate(self.neuron_list[l]):
                    ...
                    Vm = neuron.update(input_val, t)
                    if Vm == neuron.V_spike:                     #update activation map with only spikes, not memebrane potential
                        activations_list[l+1][n_id,t] = 1
